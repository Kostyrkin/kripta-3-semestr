...
class RSA:
    ...
    def C_RSA(self):
        ...
        with open(input("Введите название файла для записи открытого ключа ключа:") + ".json", "w", encoding="utf-8") as file:
            json.dump(p_k, file)

        c_k = {"privateExponent": d, "prime1": p, "prime2": q, "exponent1": pow(d, 1, (p-1)), "exponent2": pow(d, 1, (q-1)), "coefficient": pow(q, -1, p)}
        with open(input("Введите название файла для записи закрытого ключа ключа:") + ".json", "w", encoding="utf-8") as file:
            json.dump(c_k, file)

    def coding(self):
        with open(input("Введите название файла c открытым ключем:") + ".json", "r", encoding="utf-8") as file:
            p_k = json.load(file)
        e = p_k["SubjectPublicKeyInfo"]["publicExponent"]
        n = p_k["SubjectPublicKeyInfo"]["N"]
        M = self.mod_text_in_bit(input("Введите сообщение:"))
        M_m = []
        enc_M = ""
        block = self.block(n)
        ...
        for i in M_m:
            ...
            enc_M += a
        c_m = {"Version": 0, "EncryptedContentInfo": {"ContentType": "text", "ContentEncryptionAlgorithmIdentifier": "rsaEncryption", "encryptedContent": enc_M, "OPTIONAL": "NULL"}}
        with open(input("Введите название файла для записи зашифровонного текста:") + ".json", "w", encoding="utf-8") as file:
            json.dump(c_m, file)

    def decoding(self, enc_M):
        with open("zk_user.json", "r", encoding="utf-8") as file:
            c_k = json.load(file)
