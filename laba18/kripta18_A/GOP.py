...
class inditification():
    ...
    def registr(self):
        ...
        ot_ser = self.ping_server("GOTK",Mes)
        if int(ot_ser):
    ...
    def inditification(self):
        ...
        Mes = {'Action': "IDN", 'NumberOfIterations': it, "IDUser": self.id_user,
               'DisposableKeys': H}
        ot_ser = self.ping_server("IDN", Mes)
        if ot_ser == "2":
    ...
    def ping_server(self, indef, Mes):
        url = 'http://localhost:8081'

        files = json.dumps(Mes)

        resp = requests.post(url, params=indef, json=files)
...
def run():
    ...
    return inditification().inditification()