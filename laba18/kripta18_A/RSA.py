...
class RSA:
    ...
    def C_RSA(self):
        ...
        with open("zk_identification.json", "w", encoding="utf-8") as file:
            json.dump(c_k, file)

        return p_k

    def coding(self, Met, Sv, M, z_k, type_k):
        if type_k == "zk":
            d = z_k["privateExponent"]
            n = z_k["prime1"]*z_k["prime2"]
        else:
            d = z_k["SubjectPublicKeyInfo"]["publicExponent"]
            n = z_k["SubjectPublicKeyInfo"]["N"]
        A = ["512", "256"]
        B = ["GOST", "SHA"]
        M_m = []
        enc_M = ""
