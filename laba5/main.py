class SHA:
    ...
    def SHA_256(self, Mes):
        ...
        for i in range(0, len(M), 512):
            if len(M) - i < 512:
                M_512.append(M[i:] + "1" + "0"*(pow((448-len(M[i:])-1),1,512)) + "0"*(64 - len(bin(len(M))[2:])) + bin(len(M))[2:])
                if len(M) - i > 447:
        ...
        for i in M_32:
            ...
            for j in range(16, 64):
                q_1 = self.Q_1_256(W_t[j-2]) + int(str(W_t[j-7]), 2) + self.Q_0_256(W_t[j-15]) + int(str(W_t[j-16]), 2)
            ...
            # 3 этап
            for t in range(64):
                T_1 = h + self.Z_1_256(bin(e)[2:].zfill(32)) + self.Ch(bin(e)[2:].zfill(32),bin(f)[2:].zfill(32),bin(g)[2:].zfill(32)) + self.K_256[t] + W_t[t]
                T_1 = pow(T_1, 1, pow(2, 32))
                T_2 = self.Z_0_256(bin(a)[2:].zfill(32)) + self.Maj(bin(a)[2:].zfill(32),bin(b)[2:].zfill(32),bin(c)[2:].zfill(32))
        ...
        H = ""
        print(bin(self.H_256[0])[2:])
        for i in self.H_256:
            H = H + hex(i)[2:].zfill(8)
        return H


    def SHA_512(self, Mes):
        M_1024 = []
        M_64 = []
        M = self.mod_text_in_bit(Mes)
        if len(Mes) > pow(2, 128):
            return "Сообщение больше допустимого значения. Введите другое сообщение"
